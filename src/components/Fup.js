  ...
  useEffect(() => {
    (async () => {
      ...
      if (status !== 'granted') {
        ...
        setLoading(false); // Finaliza carregamento em caso de erro
        return;
      }

      let location = await Location.getCurrentPositionAsync({});
      setLocation(location.coords);
      setLoading(false); // Finaliza carregamento após obter a localização
    })();
  }, []);
  ...
  useEffect(() => {
    const fetchClinics = async (latitude, longitude) => {
      ...
      try {
        ...
        setClinics(response.data.results); // Armazena as clínicas diretamente no estado
      } catch (error) {
